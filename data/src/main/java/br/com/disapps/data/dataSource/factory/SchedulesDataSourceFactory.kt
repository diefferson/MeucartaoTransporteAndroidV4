...
class SchedulesDataSourceFactory(private val database: Database, private val restApi: RestApi, private val preferences: Preferences,private val assetManager: AssetManager) : DataSourceFactory  {

    override fun create(useCloud: Boolean): SchedulesDataSource {
        return if(useCloud){
            CloudSchedulesDataSource(restApi)
        }else{
            LocalSchedulesDataSource(database, preferences, assetManager)
        }
    }
}