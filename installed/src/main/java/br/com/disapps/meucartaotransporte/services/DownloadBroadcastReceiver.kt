...
class DownloadBroadcastReceiver : BroadcastReceiver(), KoinComponent{
        ...
        if (cursor.moveToFirst()) {
            ...
            when (status) {
                DownloadManager.STATUS_FAILED -> {
                    failDownload(referenceId, context)
                }
                DownloadManager.STATUS_SUCCESSFUL -> {
                    successDownload(referenceId, context)
                }
            }
        }else{
            failDownload(referenceId, context)
    ...
    private fun successDownload(referenceId: Long, context: Context) {
        when (referenceId) {
    ...
    private fun failDownload(referenceId: Long, context: Context) {
        when (referenceId) {
            preferences.getIdDownloadLines() -> {
                context.stopService(Intent(context, DownloadLinesService::class.java))
                deleteFromCache(SaveLinesService.FILE_PATH)
                ScheduleJob.schedule(context, ScheduleJob.LINE_TYPE)
            ...
            preferences.getIdDownloadSchedules() -> {
                ...
                ScheduleJob.schedule(context, ScheduleJob.SCHEDULE_TYPE)
                showCustomNotification(context,
